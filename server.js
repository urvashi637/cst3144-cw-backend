...
// CORS Headers with corrected spelling
...
let db;
const mongoURI = process.env.MONGO_URI || 'your-default-mongo-connection-string';
MongoClient.connect(
  mongoURI,
      ...
      if (product) {
        ...
        if (newAvailableInventory >= 0) {
          ...
          if (updateResult.modifiedCount === 0) {
            console.warn(`Failed to update inventory for subject: ${item.subject}.`);
          } else {
            console.log(`Inventory for subject ${item.subject} updated successfully.`);
          }
        } else {
          console.warn(`Not enough inventory for subject: ${item.subject}.`);
        }
      } else {
